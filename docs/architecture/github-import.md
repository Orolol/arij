...
`.git/config`, and `origin` keeps the clean URL, so later `fetch`/`pull`/`push`
behave exactly like a hand-made clone and no secret is stored on disk. Public
repositories clone with no token at all.
...
|---|---|
| A pasted string can never escape the clone root | `parseGitHubRepoInput()` re-validates owner and repo against `^[A-Za-z0-9._-]+$`, rejecting `.`, `..`, embedded traversal and a leading `-` |
...
| Only Arij-created directories are Arij's to delete | `projects.clone_source = 'github'`; `NULL` for user-supplied paths |
...
| Project creation: provenance, path normalisation | `__tests__/projects-route-post.test.ts` |
