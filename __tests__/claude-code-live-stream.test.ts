...
describe("spawnClaude with a line listener", () => {
  ...
  it("preserves split UTF-8 bytes and accepts spaced result envelopes", async () => {
    const lines: string[] = [];
    const { promise } = spawnClaude({ mode: "code", prompt: "p", onRawLine: (line) => lines.push(line) });
    const envelope = '{ "result": "Terminé", "type": "result", "subtype": "success" }';
    ...
    child.exitCode = 0;
    child.emit("close", 0);
    expect((await promise).result).toBe(envelope);
    expect(lines).toEqual([events[0], envelope]);
  });
