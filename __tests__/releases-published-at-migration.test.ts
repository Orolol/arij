/**
 * Migration 0062_release_published_at (lot 11, #105 / #109).
 *
...
const MIGRATION_TAG = "0062_release_published_at";
...
describe("0062_release_published_at", () => {
  it("sits after every migration it depends on, in apply order", () => {
    ...
    // Renumbered from 0057 at merge time, and its `when` re-stamped above
    // 0061_review_dismissal_and_git_log: under that `when`, a database that
    // had already recorded 0061 would never have applied it.
    ...
    expect(tags.indexOf(MIGRATION_TAG)).toBeGreaterThan(
      tags.indexOf("0061_review_dismissal_and_git_log")
    );
...
describe("0062_release_published_at — finalisation", () => {
