...
describe("migration journal", () => {
  ...
  it("re-applies the git_sync_log rebuild without losing rows", () => {
    // That migration was renumbered (0027 -> 0028) after it had shipped on a
    // branch, so an early database can legitimately run the rebuild twice.
    const entry = entries.find(
      (candidate) => candidate.tag === "0028_git_sync_log_nullable_project"
    );
