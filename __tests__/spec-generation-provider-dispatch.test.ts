...
describe("generate-spec provider dispatch", () => {
  beforeEach(() => {
    ...
    resetDbMockState();
    // project lookup
    dbMockState.getQueue = [{ id: "proj-1", gitRepoPath: "/tmp/repo", spec: null }, { spec: null }];
    dbMockState.allRows = [];
  ...
  it("routes claude-code through the same provider registry, not a private branch", async () => {
    ...
    expect(mocks.providerSpawn).toHaveBeenCalledTimes(1);
    // `logIdentifier` is gone with the NDJSON stream logger (removed with the
    // notifications subsystem); the mode is the half that still identifies
    // this as the spec-generation spawn.
