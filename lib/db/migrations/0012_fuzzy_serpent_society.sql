-- NOTE: chunks/sequences objects also created by 0005_codex_session_chunks (earlier
-- in the journal); IF NOT EXISTS added so the chain executes linearly.
...
CREATE UNIQUE INDEX IF NOT EXISTS `agent_session_chunks_session_sequence_unique` ON `agent_session_chunks` (`session_id`,`sequence`);--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS `agent_session_chunks_session_stream_key_unique` ON `agent_session_chunks` (`session_id`,`stream_type`,`chunk_key`);--> statement-breakpoint
CREATE INDEX IF NOT EXISTS `agent_session_chunks_session_stream_sequence_idx` ON `agent_session_chunks` (`session_id`,`stream_type`,`sequence`);--> statement-breakpoint
CREATE TABLE IF NOT EXISTS `agent_session_sequences` (
