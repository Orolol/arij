...
export class BundledPiProvider extends PiProvider {
  ...
  // The PiProvider answers the fork keeps from upstream pi. buildArgs below
  // does not use the allowlist or resume helpers (piToolPolicy and --session
  // are spelled there), but the failure labels and the contract stay pi's.
  protected get cliDisplayName(): string { return "Pi"; }

  protected readonlyTools(): string[] { return ["read", "grep", "find", "ls"]; }

  protected resumeArgs(cliSessionId: string): string[] { return ["--session", cliSessionId]; }
  ...
  buildArgs(options: ProviderSpawnOptions, context?: ProviderSpawnContext): string[] {
    ...
    args.push(...buildProviderOptionArgs(this.type, options.cliOptions));
  ...
  protected handleExit(info: ProviderExitInfo, callbacks: BaseProviderChunkCallbacks) {
    return { ...super.handleExit(info, callbacks), usage: extractPiUsage(info.stdout) };
  }
