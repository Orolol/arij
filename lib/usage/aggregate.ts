 ...
 */
function hasCodexSessions(): boolean {
  const row = db.all<{ ok: number }>(sql`
    SELECT 1 AS ok FROM agent_sessions WHERE provider = ${CODEX_PROVIDER} LIMIT 1
  `)[0];
  return Boolean(row);
}
...
function getSubscriptions(
  ...
  const snapshot = db
    ...
    .get();
  const hasCodex = hasCodexSessions();
 ...
 * counted twice as it walks `review -> done -> released`.
 *
 * Uses direct comparison to exploit the `(to_status, created_at)` index,
 * splitting the since-present and since-null queries.
 */
function getTicketsShipped(since: string | null): number {
  const row = since
    ? db.all<{ shipped: number }>(sql`
...
/**
 * One fat read for the whole Usage page.
 * `live` is optional and defaults to the no-live state so existing callers
 * and tests take the fallback paths unchanged.
 */
export function getUsageReport(
  ...
  return {
    subscriptions: getSubscriptions(live),
